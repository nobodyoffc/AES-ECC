...
//final static BigInteger HALB       	= new BigInteger("7fffffffffffffffffffffffffffffff5d576e7357a4501ddfe92f46681b20a1",16);  	// Zahl 1/2, mit dem durch 2 dividiert werden kann.
    ...
    public static BigInteger random() {
        ...
        BigInteger out = new BigInteger(256, rnd);
        while (out.compareTo(ORDNUNG) != -1 || out.compareTo(ZERO) == 0) {
            out = new BigInteger(256, rnd);
        }
        return out;
    }

    // Zq 序列化
    public static byte[] toBytes(BigInteger a) {
        byte[] out = new byte[32];

        byte[] array = a.toByteArray();
        if(array[0] == 0) {
            System.arraycopy(array, 1, out, 0, 32);
        } else {
            out = array;
        }
