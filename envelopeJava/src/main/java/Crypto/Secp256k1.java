...
// 曲线上点序列化
public static byte[] toBytes(BigInteger[] point)
{
    byte[] out = new byte[64];
    System.arraycopy(Math_Modulo.toBytes(point[0]), 0, out, 0, 32);
    System.arraycopy(Math_Modulo.toBytes(point[1]), 0, out, 32, 32);
    return out;
}

// 曲线上点反序列化
public static BigInteger[] setBytes(byte[] bytes)
{
    BigInteger[] point = new BigInteger[2];
    byte[] xBytes = new byte[32];
    byte[] yBytes = new byte[32];
    System.arraycopy(bytes, 0, xBytes, 0, 32);
    System.arraycopy(bytes, 32, yBytes, 0, 32);
    point[0] = Math_Modulo.setBytes(xBytes);
    point[1] = Math_Modulo.setBytes(yBytes);
    return point;
}
}